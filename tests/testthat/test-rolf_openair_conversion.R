...
airmo_mixed <- dplyr::bind_rows(airmo_h1, airmo_d1)
...
test_that("rolf > openair single", {
  ...
  testthat::expect_equal(
    sort(rlang::names2(oa_co)),
    sort(c("date", "unit", "CO", "site")),
    ignore_attr = TRUE
  )
...
test_that("rolf <> openair", {
  ...
  testthat::expect_equal(dim(oa_d1), c(31, 67), ignore_attr = TRUE)
  testthat::expect_equal(dim(oa_h1), c(744, 16), ignore_attr = TRUE)
  ...
  res_list <- dplyr::bind_rows(!!!purrr::map(oa_list, openair_to_rolf))
